jQuery(document).ready(function($) {
    ...
    $("li[data-dropdown=yes]").hover(function() {
        $(this).children('.dropdown-wraper').css({
            "display": "block"
        });
    ...
    $(document.body).on('appear', '.featurette', function() {
        $(this).children('.left').delay(1000).animate({
            "left": "0",
            "opacity": "1",
            "filter": "alpha(opacity = 100)"
        }, 1500);
        $(this).children('.right').delay(1000).animate({
            "right": "0",
            "opacity": "1",
            "filter": "alpha(opacity = 100)"
        }, 1500);
    ...
    slide = $('.slide'),
    button = $('.button').not('#contact-form .button'),
    mywindow = $(window),
    htmlbody = $('html,body');
...
function formSubmit() {
    ...
    var data_html = '';
    if (name == "") {
        $("#contact-form").find("label[for='name']").children('.error').css({
    ...
    if (email == "") {
        $("#contact-form").find("label[for='email']").children('.error').css({
            "display": "inline-block"
        });
    } else if (reg.test(email) == false) {
        $("#contact-form").find("label[for='email']").children('.error').html('Invalid Email!').css({
            "display": "block"
        });
        $('.email-required').html('Invalid Email!');
    } else {
        $("#contact-form").find("label[for='email']").children('.error').html('Required Field!').css({
            "display": "none"
        });
    }
    if (message == "") {
        $("#contact-form").find("label[for='message']").children('.error').css({
            "display": "inline-block"
        });
    } else {
        $("#contact-form").find("label[for='message']").children('.error').css({
        ...
        $.ajax({
            ...
            success: function(msg) {
                if (msg == 'sent') {
                    ...
                    $('#contact-success').css({
                        "color": "#EB585C"
                    }).html('Mail Error. Please Try Again.!');
                }
            }
        });
    }
