...
if (!hash.length <= 0) {
	...
	/*
	function e() {
		$.getJSON(window.home_url + "category/"+window.current_language+"/feed", function (e) {
			var n = e.channel.item[0].description;
			var r = e.channel.item[0].pubDate;
			var i = n.split("Display embed code");
			var s = r.split(" ");
			var o = s[1] + "-" + s[2] + "-" + s[3];
			var u = i[0].split('src="');
			var a = u[1].split('"');
			var f = e.channel.item[0].category;
			var l = e.channel.item[0].title;
			var c = e.channel.item[0].link;
			for (var h = 0; h < e.channel.item.length; h++) {
				$(".carousel-inner .blog-header" + h + " a").attr("href", e.channel.item[h].link);
				$(".carousel-inner .blog-header" + h + " a").text(e.channel.item[h].title);
				$("#blog-news-" + h + " .blog-img-url").attr("href", e.channel.item[h].link);
				var p = t(e.channel.item[h].title, 46);
				$("#blog-news-" + h + " h3 a").text(p);
				$("#blog-news-" + h + " h3 a").attr("href", e.channel.item[h].link);
				var d = t($(e.channel.item[h].description).text(), 96);
				$("#blog-news-" + h + " .blog-news-info").text(d);
				$("#blog-news-" + h + " .blog-news-more").attr("href", e.channel.item[h].link);
				var v = e.channel.item[h].description;
				var m = v.split("Display embed code");
				var g = m[0].split('src="');
				var y = g[1].split('"');
				$("#blog-news-" + h + " .blog-img-holder").attr("style", "background:url(" + y[0] + ")");
				var b = e.channel.item[h].pubDate;
				var w = b.split(" ");
				var E = w[1] + "-" + w[2] + "-" + w[3];
				$("#blog-news-" + h + " .blog-news-date").text(E)
			}
			$(".blog-latest .news-title a").text(t(e.channel.item[0].title, 45));
			$(".blog-latest .news-title a").attr("href", c);
			$(".blog-latest .news-date div").text(o);
			$(".blog-latest .news-desc").text(t($(e.channel.item[0].description).text(), 100));
			$(".blog-latest .news-more a").attr("href", c)
		})
	}
	**/
...
//	});
	...
	$('#cta-btn, a:contains("Contact")').click(function () {
