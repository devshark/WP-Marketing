...
jQuery(window).load(function() {
	...
	//Testimonials
	var $container = $('.eva-testimonials.columns ul, .eva-blog.masonry, .eva-blog.full_width');

	$container.isotope({
		itemSelector : '.testimonial-item, .type-post',
		masonry: {columnWidth: 1}
	});
	
	
	//Gallery
	var $container = $('.gallery-columns');
    ...
    //Portfolio 
    jQuery('.eva-portfolio.wrap_container, .eva-portfolio.full_width').each(function(i){
        var $currentPortfolio=jQuery(this);
        var $currentInfinite=$currentPortfolio.children('.eva-infinite-scroll');
        var $currentIsotopeContainer=$currentPortfolio.children('.row').children('.isotope-container');
         // Isotop
        showFilter($currentIsotopeContainer.closest('.row').prev('.eva-filter').children('.filters'),$currentIsotopeContainer.children('article'));
        ...
        $currentPortfolio.find('ul.filters').each(function() {
            ...
            $optionLinks.click(function() {
                var $container2 = jQuery(this).closest('.eva-portfolio').find('.isotope-container');
            ...
            if($currentInfinite.attr('data-has-next')==='true'&&$currentNextLink.hasClass('next')){
                ...
                jQuery.ajax({
                    ...
                    success: function(response){
                        var $newElements = jQuery(response).find('.eva-portfolio').eq(i).children('.row').children('.isotope-container').html();
                        var $newURL      = jQuery(response).find('.eva-portfolio').eq(i).find('.eva-infinite-scroll>a.next').attr('href');
                        var $hasNext     = jQuery(response).find('.eva-portfolio').eq(i).find('.eva-infinite-scroll').attr('data-has-next');
                        if($newElements){
