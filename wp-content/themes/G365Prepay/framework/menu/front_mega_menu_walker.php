<?php
class evatheme_Nav_Menu_Walker extends Walker_Nav_Menu {
	private $_last_ul = '';
	...
	function display_element($element, &$children_elements, $max_depth, $depth = 0, $args, &$output) {
		...
		if (is_object($args[0])) {
			...
			$args[0]->is_bordered = ($element->_evatheme_mega_menu_bordered==1);
		}

		if (
			$depth==0 &&
			$element->_evatheme_mega_menu_enabled==1 && 
			!empty($children_elements[$element->$id_field])
		) {
			$columns = $element->_evatheme_mega_menu_columns;
...
//		if ($item->ID==4798) {var_dump($item->classes);}
		...
		if (!empty($item->_evatheme_mega_menu_subtitle)  && ($depth>0)) {
		...
		// if Mega Menu
		if ($item->_evatheme_mega_menu_enabled==1) {
		...
		$output .= $indent . '<li id="nav-menu-item-' . $item->ID . '" class="' . $mega_menu_class . ' ' . $depth_class_names . '">';
		...
		$icon = !empty($item->_evatheme_mega_menu_icon) ? '<i class="fa '.$item->_evatheme_mega_menu_icon.'"></i>' : '';
		if (!empty($item->_evatheme_mega_menu_subtitle) && ($depth>0)) {
			$subtitle = !empty($icon) ? '<span class="subtitle has-icon">'.$item->_evatheme_mega_menu_subtitle.'</span>' : '<span class="subtitle">'.$item->_evatheme_mega_menu_subtitle.'</span>';
		} else {
			$subtitle = '';
		}
